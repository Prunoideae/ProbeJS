...
public class ComponentConverter {
    public static PropertyType<?> fromDescription(TypeDescJS description) {
        if (description instanceof ArrayDescJS array) {
            ...
            if (parameterized.type() instanceof PrimitiveDescJS primitive && primitive.type().endsWith(TagKey.class.getName()) && parameterized.types()[0] instanceof PrimitiveDescJS registryType) {
                // If we have a TagKey<RegistryType>, we can use the special type generated by ProbeJS
                // Note how endsWith is used here because the type context will add $probejs$ to the start of the type
                try {
