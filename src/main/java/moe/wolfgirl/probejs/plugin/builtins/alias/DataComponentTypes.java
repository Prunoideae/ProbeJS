...
public class DataComponentTypes extends ProbeJSPlugin {
    ...
    private void patchComponentFunctions(ClassDecl classDocument) {
    ...
    private void patchItemBuilder(ClassDecl classDocument) {
        classDocument.members.removeIf(m -> m instanceof MethodDecl md && md.name.equals("component"));
        var component = Members.method("component")
                .typeParam("T", Types.wrapped("keyof %s", INPUT_MAP))
                .param("type", Types.variable("T"))
                .param("data", Types.wrapped("%s[T]", INPUT_MAP))
                .returnType(Types.THIS)
                .build();
        component.addComments("Add a default component to the item. Can be used for attribute modifiers, default enchantments... and so on.");
        classDocument.members.add(component);
    }
    ...
    public void transformClass(Documents.ClassDocument document) {
        ...
        if (ComponentFunctions.class.isAssignableFrom(classInfo.clazz())) patchComponentFunctions(classDocument);
        if (classInfo.clazz() == ItemBuilder.class) patchItemBuilder(classDocument);
    }
