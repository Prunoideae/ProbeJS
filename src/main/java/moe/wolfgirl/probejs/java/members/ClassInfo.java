...
public record ClassInfo(
    ...
    private static List<MethodInfo> findMethods(Class<?> clazz, Map<String, TypeInfo> variableRemaps) {
        CachedClassInfo classInfo = CachedClassStorage.GLOBAL_PUBLIC.get(clazz);
        try {
            List<MethodInfo> methods = classInfo.getAccessibleMethods(false)
                    .stream()
            ...
            Set<MethodInfo> syntheticToRemove = new HashSet<>();
            List<MethodInfo> methodsToAdd = new ArrayList<>();
            for (MethodInfo method : methods) {
                if (method.isSynthetic()) {
                    // Find if another non-synthetic method with same name and number of params exists
                    // Synthetic methods have all object params, we can't use param class to match
                    boolean hasNonSynthetic = methods.stream().anyMatch(m -> !m.isSynthetic() &&
                            m.name().equals(method.name()) &&
                            m.params().size() == method.params().size()
                    );
                    if (hasNonSynthetic) syntheticToRemove.add(method);
                    else {
                        MethodInfo source = findSourceOfSynthetic(method, clazz, variableRemaps);
                        if (source != null) {
                            syntheticToRemove.add(method);
                            methodsToAdd.add(source);
                        }
                    }
                }
            }
            methods.addAll(methodsToAdd);
            methods.removeAll(syntheticToRemove);
            return methods;
        } catch (Throwable t) {
            return List.of();
        }
    ...
    // Searches for the original method that a synthetic method delegates to, so that we can reuse the method signature
    // in the dump instead of dumping the synthetic method with all object params.
    @Nullable
    public static MethodInfo findSourceOfSynthetic(MethodInfo syntheticMethod, Class<?> clazz, Map<String, TypeInfo> variableRemaps) {
        if (!syntheticMethod.isSynthetic()) {
            return syntheticMethod;
        }

        for (CachedMethodInfo.Accessible m : CachedClassStorage.GLOBAL_PUBLIC.get(clazz).getAccessibleMethods(false)) {
            Method method = m.getInfo().getCached();
            if (!method.isSynthetic() &&
                    method.getName().equals(syntheticMethod.name()) &&
                    method.getParameterCount() == syntheticMethod.params().size()
            ) {
                return MethodInfo.resolve(m.getInfo(), variableRemaps);
            }
        }

        for (Class<?> i : clazz.getInterfaces()) {
            if (i == Object.class) continue;
            MethodInfo source = findSourceOfSynthetic(syntheticMethod, i, variableRemaps);
            if (source != null) {
                return source;
            }
        }
        ...
        if (superClass != null && superClass != Object.class) {
            return findSourceOfSynthetic(syntheticMethod, superClass, variableRemaps);
        }
        return null;
    }
    ...
    // FIXME: Using string will cause issues if there are multiple type variables with the same name in the hierarchy.
    // However, it is impossible to fix because TypeInfo erased TypeVariable and only keeps the name and the bound
